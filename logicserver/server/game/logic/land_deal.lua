...
function cardTool.getCardType(cards)
	...
	if analayse_data.blockcount[3] > 0 then
		...
		if analayse_data.blockcount[3] * 5 == cardcount then return cardTool.CT_THREE_LINE_TWO end	--三带一对飞机
...
function cardTool.removeCards(removecards, playercards)
	...
	if rcount > pcount or pcount > 20 then return false end
	...
	if deletecount ~= rcount then return false end
...
--出牌搜索
function cardTool.searchCards(allcards, turncards, teammate)
	--排列扑克
	...
	elseif turntype == cardTool.CT_SINGLE or turntype == cardTool.CT_DOUBLE or 
		   turntype == cardTool.CT_THREE or turntpe == cardTool.CT_BOMB_CARD then
		--单牌/对牌/三条
		local refercard = turncards[1]

		--若上家是队友且出的牌大于Q直接pass
		if teammate and 
		   (turntpe == cardTool.CT_BOMB_CARD or cardTool.getCardLogicValue(refercard) >= PASS_LIMIT_CARD)then
			return {}
		end
		...
		elseif turntype == cardTool.CT_THREE then
			samecount = 3
		elseif turntpe == cardTool.CT_BOMB_CARD then
			samecount = 4
		end
		tmpreslut = cardTool.searchSameCard(allcards, refercard, samecount, teammate)
		if #tmpreslut > 0 then
			resultcards = tmpreslut[1]
			return resultcards
		end
	...
	elseif teammate then
		LOG_DEBUG("teammate play. need pass")
		return {}
...
--同牌搜索
function cardTool.searchSameCard(cards, refercard, samecount, teammate)
	--排列扑克
	cardTool.sortCards(cards)
	--分析扑克
	local analayse_data = cardTool.getAnalyse()
	...
	if #result > 0 and #result[1] > samecount and not teammate then
		local ret = {}
		for i=1, #result do
			if i==1 then
				ret[i] = {}
