...
local TIME_MASTER = 3 --叫地主时间
local TIME_OUTCARD = 10 --出牌时间
...
--出牌
local function out_card(p, outcards, client_card_type)
    ...
    if not client_card_type or client_card_type ~= cardtype or cardtype == land_deal.CT_ERROR then 
        LOG_DEBUG("player[%d] out card faild", p.uid)
        return 
    end
    --对比扑克
    if turn_count ~= 0 then
        if land_deal.compareCard(turn_cards, outcards, turn_count) == false then return end
    end

    --删除扑克
    if land_deal.removeCards(outcards, p.cards) == false then return end
...
function this.dispatch(p, name, msg)
    ...
    elseif name == "SetRate" then
        if game_status == 2 and hasstart and p.can_master and p.seatid and p.seatid > 0 and p.ready == 1 and gametype == 2 then
            if msg.rate > 3 or msg.rate < 0 or (msg.rate <= masterscore and masterscore > 0 and msg.rate > 0) then return end
            game_status = 102
            p.can_master = false

            LOG_DEBUG(p.uid .. "叫分:" .. p.rate)
    ...
    elseif name == "PlayCard" then
        ...
        if game_status == 3 and hasstart and p.seatid and p.seatid > 0 and p.ready == 1 then
