var Define = {


//	常量定义
    CMD_MD5_KEY: "~~663dskojfasjfk44..~~",

//内核命令
    MDM_KN_COMMAND: 0,						//内核命令
    SUB_KN_DETECT_SOCKET: 1,			//检测命令
    SUB_KN_VALIDATE_SOCKET: 2,				//验证命令
    MDM_KN_COMMAND_LAJI: 65534,				//内核命令
    MDM_KN_COMMAND_LAJI2: 65533,		//内核命令
    MDM_KN_COMMAND_LAJI3: 65532,

    SOCKET_TCP_BUFFER: 16384,			//网络缓冲
    ID_CREATE_EXE: 2060,

//发送映射
    g_SendByteMap: [
        
    ],

//接收映射
    g_RecvByteMap: [
        
    ],

//数据类型
    DK_MAPPED: 0X01,					//映射类型
    DK_ENCRYPT: 0X02,					//加密类型
    DK_COMPRESS: 0X04,					//压缩类型

//机器标识
    LEN_MACHINE_ID: 33,	//序列长度
    LEN_NETWORK_ID: 13,	//网卡长度

//资料数据
    LEN_MD5: 33,	//加密密码
    LEN_ACCOUNTS: 32,	//备注长度
    LEN_PASS_PORT_ID: 19,                //证件号码长度
    LEN_NICENAME: 32,	//帐号长度
    LEN_NICKNAME: 32,	//昵称长度
    LEN_PASSWORD: 33,	//密码长度
    LEN_GROUP_NAME: 32,	//社团名字
    LEN_UNDER_WRITE: 32,	//个性签名

//数据长度
    LEN_QQ: 16,	//q q号码
    LEN_EMAIL: 33,	//电子邮件
    LEN_USER_NOTE: 256,	//用户备注
    LEN_SEAT_PHONE: 33,	//固定电话
    LEN_MOBILE_PHONE: 12,	//移动电话
    Len_PASS_PORT_ID: 19,	//证件号码
    LEN_COMPELLATION: 16,	//真实名字
    LEN_DWELLING_PLACE: 128,	//联系地址

    LEN_QUESTION: 256,
    LEN_ANSWER: 30,
    LEN_LUCKY_RESULT: 10,

//机器标识
    LEN_MACHINE_ID: 33,	//序列长度
    LEN_NETWORK_ID: 13,	//网卡长度

//列表数据
    LEN_TYPE: 32,	//种类长度
    LEN_KIND: 32,	//类型长度
    LEN_NODE: 32,	//节点长度
    LEN_PAGE: 32,	//定制长度
    LEN_SERVER: 32,	//房间长度
    LEN_PROCESS: 32,	//进程长度

//游戏状态
    GAME_STATUS_FREE: 0,	//空闲状态
    GAME_STATUS_PLAY: 100,	//游戏状态
    GAME_STATUS_WAIT: 200,	//等待状态
    GAME_STATUS_ENDED: 255,                     //结束状态

//系统参数
    LEN_USER_CHAT: 128,	//聊天长度
    TIME_USER_CHAT: 1,	//聊天间隔
    TRUMPET_MAX_CHAR: 128,            //喇叭长度

//验证码取数位置
    yueding1: 7,
    yueding2: 53,
    yueding3: 88,
    yueding4: 122,

//携带信息

//其他信息
    DTP_GR_TABLE_PASSWORD: 1,//桌子密码

//用户属性
    DTP_GR_NICK_NAME: 10,	//用户昵称
    DTP_GR_GROUP_NAME: 11,	//社团名字
    DTP_GR_UNDER_WRITE: 12,	//个性签名
    DTP_GR_USER_QQ: 13,

    US_NULL: 0,	//没有状态
    US_FREE: 1,	//站立状态
    US_SIT: 2,	//坐下状态
    US_READY: 3,	//同意状态
    US_LOOKON: 4,	//旁观状态
    US_PLAYING: 5,	//游戏状态
    US_OFFLINE: 6,	//断线状态

//参数定义
    INVALID_CHAIR: 65535,//无效椅子
    INVALID_TABLE: 65535,//无效桌子
    INVALID_USERID: 0,	//无效用户

//变化原因
    SCORE_REASON_WRITE: 0,//写分变化
    SCORE_REASON_INSURE: 1,        		//银行变化
    SCORE_REASON_PROPERTY: 2,//道具变化
    SCORE_REASON_MATCH_FEE: 3,//比赛报名
    SCORE_REASON_MATCH_QUIT: 4,//比赛退赛

//答题消息
    MDM_MAIN_LOGON_QUESTION: 10,

    SUB_LOGON_QUESTION: 9,
    SUB_QUESTION_ANSWER: 8,
    SUB_REWARDS_COUNT: 7,				//获得奖励用户
    SUB_REWARDS_RESULT: 6,				//写奖励

    MDM_MISSION_QUESTION: 20,
    SUB_MISSION_C_QUESTION: 19,
    SUB_MISSION_SUCCESS: 18,
    SUB_MISSION_COUNT: 17,
    SUB_MISSION_C_REWARDS: 16,			//随机奖励数量
    SUB_MISSION_C_LUCK_RESULT: 15,		//转盘是否开始

    PROPERTY_ID_TRUMPET: 19,			//喇叭道具
    PROPERTY_ID_TYPHON: 20,		//喇叭道具

}
module.exports = Define;